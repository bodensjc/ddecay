...
Bool_t dsp_background::Process(Long64_t entry)
{
 fReader.SetLocalEntry(entry);//do NOT delete this line, otherwise the program will crash
  ...
  //fill signal bars
  if (*Dsplus_M > 1958 && *Dsplus_M < 1980) {
    phisighist->Fill(mkpkm);
   probsighist->Fill(probprod);
   fdchi2sighist->Fill(TMath::Log(*Dsplus_FDCHI2_OWNPV));
   ipchi2sighist->Fill(*Dsplus_IPCHI2_OWNPV);
   mdspsig->Fill(*Dsplus_M);
  }
  //fill (outer) background bars
  if ((*Dsplus_M < 1940 && *Dsplus_M > 1918) || (*Dsplus_M < 2020 && *Dsplus_M > 1998)) {
    phiosbhist->Fill(mkpkm);
    probosbhist->Fill(probprod);
    fdchi2osbhist->Fill(TMath::Log(*Dsplus_FDCHI2_OWNPV));
    ipchi2osbhist->Fill(*Dsplus_IPCHI2_OWNPV);
    mdsposb->Fill(*Dsplus_M);
  }

//inner side band
  if ((*Dsplus_M < 1959 && *Dsplus_M > 1939) || (*Dsplus_M > 1979 && *Dsplus_M < 1999)) {
    mdspisb->Fill(*Dsplus_M);
  };
