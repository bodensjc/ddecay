j = Job(name='6-24:1 DpDsp_KKpi') #"mm-dd:run decay"
myApp = GaudiExec()
...
j.inputdata = data[0:1] #get first 1 data files (out of 4311)
j.backend = Dirac()
j.outputfiles = [
	DiracFile('DpDsp2KKpiMagDown.root')
]
j.splitter = SplitByFiles(filesPerJob=1) #should give 1 subjobs (1 files / 1 files per job)
j.submit()
